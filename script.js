...
async function trackOrder() {
    ...
    if (!orderId) {
        errorBox.innerText = "Please enter your Order ID.";
        return;
    }

    const { data, error } = await supabaseClient
    .rpc("track_customer_order", {
        p_order_id: orderId
    })
    .maybeSingle();
    ...
    if (error) {
        console.error(error);
        errorBox.innerText = "Unable to track order. Please try again.";
        return;
    }

    if (!data) {
        errorBox.innerText = "Order not found. Please check your Order ID.";
        return;
    }
...
resultBox.innerHTML = `
    ...
    <h3>Order Status: ${currentStatus}</h3>
